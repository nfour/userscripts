...
exports.extractProjectAndJobIdsFromUrl = extractProjectAndJobIdsFromUrl;
function reuseElement({ existing, html }) {
    return existing && existing.length
        ? existing
        : $(html);
}
exports.reuseElement = reuseElement;
...
class JobTraceView {
    constructor({ pollingRate = 5000, $container }) {
        ...
        this.$jobTraceView = lib_1.reuseElement({
            html: `
        <div class="inactive ${constants_1.classes.JobTraceView}">
          <span class="${constants_1.classes.JobTraceClose}">X</span>
        </div>
      `,
            existing: $container.find(`.${constants_1.classes.JobTraceView}`),
        });
        const $closeButton = this.$jobTraceView.find(`.${constants_1.classes.JobTraceClose}`);
    ...
    close() {
        this.$jobTraceView
            .removeClass('inactive')
            .addClass('inactive')
            .find(`.${constants_1.classes.JobTrace}`).remove();
    }
    disable() {
        this.stopUpdates();
        this.close();
...
exports.JobTraceView = JobTraceView;
/**
...
const constants_1 = __webpack_require__(/*! ../constants */ "./src/gitlabPipelines/constants.ts");
const JobTraceView_1 = __webpack_require__(/*! ./JobTraceView */ "./src/gitlabPipelines/views/JobTraceView.ts");
const getSelectorForElement = new simmerjs_1.default();
function setupInputOnPipelineWidgets($pipelineWidgets) {
    $pipelineWidgets.each((i, widget) => {
        setupInputsOnPipelineList($(widget));
    });
}
exports.setupInputOnPipelineWidgets = setupInputOnPipelineWidgets;
function setupInputsOnPipelineList($container) {
    const jobTraceView = new JobTraceView_1.JobTraceView({ $container, pollingRate: 2000 });
    function addButtonToList(row) {
        const $row = $(row);
        const url = $row.find(constants_1.paths.pipelineDropdownJobButton).attr('href');
    ...
    const observer = new selector_observer_1.default(widget);
    const selectorForListItems = `${getSelectorForElement(widget)} ${constants_1.paths.pipelineWidgetListItem}`;
...
function pipelineViewButton() {
    return $(`
    <span class="${constants_1.classes.OpenPipelineButton}"></span>
  `);
}
